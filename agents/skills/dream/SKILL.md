...
After each direct JSONL capture has been successfully triaged (including discard or hold), run `python3 ~/.claude/skills/dream/scripts/sessions.py mark --source <claude|codex> --session-id <id> --turn <number>`. This local checkpoint prevents reingestion after inbox deletion while allowing a resumed session's later turns to be processed. Never mark a turn whose KB write or triage failed. Under `--dry-run`, neither ingest nor mark. Session captures contain user requests and final responses only; verify claims against durable outcomes before merging them into topical docs.
